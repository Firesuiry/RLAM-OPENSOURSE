...
class FT1PsoSwarm(MatSwarm):
    optimizer_name = 'FT1PSO'
    action_space = 5
...
def init_fuzzy_system():
    ...
    input_low = T1FS(domain, tri_mf, [-0.5, 0, 0.5, 1])
    input_medium = T1FS(domain, tri_mf, [0, 0.5, 1, 1])
    input_high = T1FS(domain, tri_mf, [0.5, 1, 1.5, 1])

    output_low = T1FS(domain, tri_mf, [0, 0.5, 1, 1])
    output_medium_low = T1FS(domain, tri_mf, [0.5, 1, 1.5, 1])
    output_medium = T1FS(domain, tri_mf, [1, 1.5, 2, 1])
    output_medium_high = T1FS(domain, tri_mf, [1.5, 2, 2.5, 1])
    output_high = T1FS(domain, tri_mf, [2, 2.5, 3, 1])
...
def get_c1c2(iteration, diversity, error):
    ...
    s, c = fuzzy_system.evaluate({"iteration": iteration, "diversity": diversity, "error": error})
    return (c['C1'], c['C2'])


if __name__ == '__main__':
    s = FT1PsoSwarm(1000, 40, True, fun, 10, 1000, -1000, {})
    s.run()
