...
class AwpsoSwarm(MatSwarm):
    optimizer_name = 'AWPSO'
    action_space = 5
    ...
    def run_once(self, actions=None):
        ...
        for i in range(self.n_part):
            delta_p_best = self.p_best[i] - self.xs[i]
            delta_g_best = self.history_best_x - self.xs[i]
            c1 = 0.5 / (1 + np.exp(-0.000035*200*delta_p_best)) + 1.5
            c2 = 0.5 / (1 + np.exp(-0.000035*200*delta_g_best)) + 1.5
...
if __name__ == '__main__':
    s = AwpsoSwarm(1000, 100, True, fun, 10, 1000, -1000, {})
    s.run()
