...
class QrlepsoBaseSwarm(MatSwarm):
    optimizer_name = 'Q-RELPSO-BASE'
    action_space = 10
    ...
    def quantization_random(self, data_range):
        return self.data_quantization(np.random.random(), data_range)

    def run_once(self, actions=np.zeros(50)):
        if self.show:
            print('{}|best fit:{}'.format(self.fe_num / self.fe_max, self.history_best_fit))
        actions = self.data_quantization(actions, 1)
        ...
        for i in range(self.n_part):
            ...
            for d in range(self.n_dim):
                ...
                clpso_delta_v = r1 * self.quantization_random(2 * self.pos_max) * self.data_quantization(clpso_target - location,
                                                                                         2 * self.pos_max)
                # fdr_delta_v = r2 * self.quantization_random() * self.data_quantization(fdr_target - location,
                #                                                                        2 * self.pos_max)
                ...
                gbest_delta_v = r5 * self.quantization_random(2 * self.pos_max) * self.data_quantization(self.g_best[d] - location,
                                                                                         2 * self.pos_max)
                pbest_delta_v = r6 * self.quantization_random(2 * self.pos_max) * self.data_quantization(self.p_best[i, d] - location,
                                                                                         2 * self.pos_max)
            ...
            if np.random.random() < mutation_rate * self.flag[i]:
                self.xs[i] = np.float16(np.random.uniform(self.pos_min, self.pos_max, self.xs[i].shape))

        self.vs[self.vs > self.max_v] = self.max_v
        self.vs[self.vs < self.min_v] = self.min_v
        assert np.max(np.abs(self.vs)) <= self.max_v, F'max v:{self.max_v}'
        self.vs = self.data_quantization(self.vs, self.max_v)
        ...
        self.xs = self.data_quantization(self.xs, self.pos_max)
        self.fits = self.fun(self.xs)
