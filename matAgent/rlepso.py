...
class RlepsoSwarm(TestpsoSwarm):
    ...
    def __init__(self, n_run, n_part, show, fun, n_dim, pos_max, pos_min, config_dic):
        ...
        model = config_dic.get('model', r"D:\develop\swam\ieeeaccess - 副本 - 副本 - 副本 - 副本\rl\train0\ddpg_actor_episode100.h5")
    ...
    def run_once(self, action=np.zeros(10)):
        ...
        if self.show:
            ...
            print(action[:10])
        super(RlepsoSwarm, self).run_once(action.numpy())
...
if __name__ == '__main__':
    ...
    s = RlepsoSwarm(1000, 40, True, fun2, 2, 100, -100, {'max_fes': 10000,
                                                        'model': model})
    s.run()
